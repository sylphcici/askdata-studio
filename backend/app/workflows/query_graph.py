...
class QueryWorkflow:
    ...
    def _compile(self):
        ...
        builder.add_conditional_edges(
            "preprocess",
            lambda state: "human_clarification" if state.get("clarification") else {
    ...
    def _human_clarification(self, state: QueryState) -> dict[str, Any]:
        ...
        selected_option = next(
            (item for item in payload.get("options") or [] if str(item.get("id")) == option_id),
            {},
        )
        tables = list(workspace.get("confirmed_schema_tables") or [])
        is_schema_table_option = any(table["id"] == option_id for table in SCHEMA)
        if is_schema_table_option:
        ...
        if parameter == "metric" and selected_option:
            label = str(selected_option.get("label") or option_id)
            extraction = dict(state.get("extraction") or {})
            extraction["metrics"] = [label]
            extraction["retrieval_terms"] = list(dict.fromkeys([
                *list(extraction.get("retrieval_terms") or []),
                label,
            ]))
            update["extraction"] = extraction
            update["standalone_query"] = f"{state.get('standalone_query') or state.get('query')}，指标使用{label}"
            update["rewritten"] = True
        elif selected_option and not is_schema_table_option:
            label = str(selected_option.get("label") or option_id)
            description = str(selected_option.get("description") or "").strip()
            selected_semantics = f"{label}（{description}）" if description else label
            extraction = dict(state.get("extraction") or {})
            extraction["retrieval_terms"] = list(dict.fromkeys([
                *list(extraction.get("retrieval_terms") or []),
                label,
            ...
            update["extraction"] = extraction
            update["standalone_query"] = (
                f"{state.get('standalone_query') or state.get('query')}，"
                f"用户已选择口径：{selected_semantics}"
            )
            update["rewritten"] = True
        return update
    ...
    def _prepare_single_database(self, state: QueryState) -> dict[str, Any]:
        ...
        previous_sql = ""
        if state.get("rewritten") and state.get("recent_result_context"):
            try:
                previous_sql = str(json.loads(state["recent_result_context"]).get("sql") or "")
        ...
        if decision["action"] == "clarify":
            clarification = decision["clarification"]
            if self._is_technical_clarification(clarification):
                detail = str(clarification.get("reason") or clarification.get("question") or "")
                return {
                    ...
                    "mcp_execution": {
                        ...
                        "rows": [],
                        "error": (
                            "数据模型缺少可靠的关联关系，无法完成本次查询。"
                            f" 技术详情：{detail}" if detail else
                            "数据模型缺少可靠的关联关系，无法完成本次查询。"
                        ),
                    },
                    "mcp_tool_trace": decision.get("tool_trace", []),
                    "direct_sql": "",
                    "sql_source": "technical_clarification_blocked",
                }
            return {
                "workflow_mode": "single_database_agent",
    ...
    @staticmethod
    def _is_technical_clarification(clarification: dict[str, Any]) -> bool:
        options = clarification.get("options") or []
        text = " ".join([
            str(clarification.get("parameter") or ""),
            str(clarification.get("question") or ""),
            str(clarification.get("reason") or ""),
            *[
                " ".join([
        ...
        return any(term in text for term in (
            "join", "schema", "关联字段", "关联关系", "关联定义",
            "外键", "foreign key", "允许关联", "技术字段",
        ))
    ...
    def _execute_single_database(self, state: QueryState) -> dict[str, Any]:
        ...
        execution = SqlExecution(
            ...
            total_row_count=int(raw_execution.get("total_row_count") or len(raw_execution.get("rows") or [])),
            truncated=bool(raw_execution.get("truncated")),
        )
