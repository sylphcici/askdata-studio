...
class SessionContext:
    ...
    def delete_conversation(self, user_id: str, conversation_id: str) -> bool:
        ...
        for task_id in self.session_tasks.pop(scoped_session_id, []):
            task = self.tasks.get(task_id)
            if task and task.get("user_id") == user_id:
                self.tasks.pop(task_id, None)
        self.short_term_memory.discard(scoped_session_id)
        return self.archive.delete_conversation(user_id, conversation_id)
    ...
    def _route_item(item: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "standalone_query": str(item.get("standalone_query") or ""),
    ...
    def _memory_items(self, session_id: str) -> list[dict[str, Any]]:
        ...
        for task_id in self.session_tasks.get(session_id, []):
            ...
            output.append({
                ...
                "standalone_query": result.standalone_query or "",
                "interpretation": (
                    result.interpretation.model_dump(mode="json")
                    if result.interpretation else None
                ),
                "columns": result.columns,
                "row_count": len(result.rows),
            })
    ...
    def match_clarification(text: str, result: QueryResult) -> str | None:
        ...
        recommendation_replies = {
            "都行", "都可以", "随便", "默认", "用默认的",
            "你推荐", "你推荐吧", "按推荐", "按推荐的", "推荐一个",
        }
        if normalized in recommendation_replies:
