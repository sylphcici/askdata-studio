...
from .excel_export import build_excel_xml
...
class AskDataService:
    ...
    def __init__(
        ...
        self.memories = MemoryStore(
            path=self.config.saved_memories_file,
            limit=self.config.max_saved_memories,
        )
    ...
    def export_task(self, task_id: str, user_id: str | None = None) -> tuple[str, bytes, int]:
        result = self.get_task(task_id, user_id)
        if result.status != "completed" or result.route != "database_query" or not result.sql:
            raise ValueError("该任务没有可导出的查询结果")
        database = next(
            (
                str(call.get("database"))
                for call in reversed(result.tool_calls)
                if call.get("database") and call.get("success")
            ),
            "",
        )
        if not database:
            raise ValueError("无法确定查询使用的数据库")
        scope = self.access_controller.resolve(user_id)
        execution = self.workflow.database_engine.execute_full(
            database, result.sql, scope, max_rows=50_000
        )
        if not execution.success:
            raise ValueError(f"完整结果查询失败：{execution.error}")
        if execution.truncated:
            raise ValueError("完整结果超过50000行，请缩小查询范围后再导出")
        title = result.result_title or "查询结果"
        content = build_excel_xml(title, execution.columns, execution.rows)
        return title, content, len(execution.rows)
