...
evaluation_reports = EvaluationReportService()
...
def list_evaluations(_: AuthUser = Depends(require_admin)) -> list[dict]:
    return evaluation_reports.list_reports()


@router.get("/admin/evaluations/{report_id}")
def evaluation_detail(
    report_id: str,
    _: AuthUser = Depends(require_admin),
) -> dict:
    try:
        return evaluation_reports.get_report(report_id)
    except KeyError as error:
        raise HTTPException(status_code=404, detail="评测报告不存在") from error
...
def conversations(user: AuthUser = Depends(require_user)) -> list[dict]:
    return service.context.archive.list_conversations(user.user_id)


@router.put("/conversations/{conversation_id}", response_model=ConversationRecord)
def save_conversation(
    ...
    if conversation_id != payload.id:
        raise HTTPException(status_code=400, detail="对话标识不一致")
    service.context.archive.save_conversation(
        user.user_id,
...
def delete_conversation(
    ...
    deleted = service.context.delete_conversation(user.user_id, conversation_id)
...
def export_task(
    ...
    try:
        title, content, row_count = service.export_task(task_id, user.user_id)
    except KeyError as error:
        raise HTTPException(status_code=404, detail="查询任务不存在") from error
    except PermissionError as error:
        raise HTTPException(status_code=403, detail=str(error)) from error
