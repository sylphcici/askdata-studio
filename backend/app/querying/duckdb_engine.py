...
class DuckDbEngine:
    ...
    def execute(
        ...
        try:
            ...
            with self.connect(database) as connection:
                total_row_count = int(
                    connection.execute(
                        f"SELECT COUNT(*) FROM ({safe_sql}) AS askdata_count"
                    ).fetchone()[0]
                )
    ...
    def execute_full(
        ...
        try:
            ...
            return SqlExecution(
                safe_sql, True, columns, rows,
                total_row_count=len(raw_rows) if not truncated else max_rows + 1,
                truncated=truncated,
            )
        except (ValueError, ParseError, duckdb.Error, OSError) as exc:
            return SqlExecution(sql, False, error=str(exc))
    ...
    def _validate_sql(
        ...
        database_tables = [
            table
            for table in SCHEMA
            if table.get("database", "askdata_mock") == database
        ]
        allowed = {
