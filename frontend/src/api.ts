import type { AuthUser, ConversationRecord, EvaluationReport, EvaluationReportSummary, LoginResponse, QueryResult, SavedMemory, SchemaField, SchemaTable, WorkspaceConfig } from "./types"
...
export const api = {
  ...
  conversations: () => request<ConversationRecord[]>("/api/conversations"),
  saveConversation: (conversation: ConversationRecord) =>
    request<ConversationRecord>(`/api/conversations/${encodeURIComponent(conversation.id)}`, {
      method: "PUT",
  ...
  exportTask: (taskId: string) =>
    download(`/api/tasks/${encodeURIComponent(taskId)}/export`),
  save: (taskId: string) =>
  ...
  evaluationReports: () => request<EvaluationReportSummary[]>("/api/admin/evaluations"),
  evaluationReport: (reportId: string) =>
    request<EvaluationReport>(`/api/admin/evaluations/${encodeURIComponent(reportId)}`),
}
